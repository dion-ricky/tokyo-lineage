...
from utils.airflow import get_dagbag, get_task_instances_from_dagrun, \
    get_dag_from_dagbag, get_task_from_dag, instantiate_task_from_ti
...
class AirflowExtractor(BaseExtractor):
    ...
    def handle_job_run(self, job: DagRun):
        ...
        for task_instance in task_instances:
            _task = get_task_from_dag(dag, task_instances.task_id)
            _task, _ = instantiate_task_from_ti(_task, task_instance)

            self._handle_task_run(_task, task_instance)
    
    def _handle_task_run(self, task, task_instance):
        # new models.airflow_task
        # handle_task_run
        pass
